...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            wj, wi = self.stack.pop(), self.stack.pop()
            self.stack.append(wj)
            self.dependencies.append((wj, wi))
        elif transition == 'RA':
            wj, wi = self.stack.pop(), self.stack.pop()
            self.stack.append(wi)
            self.dependencies.append((wi, wj))
        else:
            raise ValueError('transition not recogonized')
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        curr_parses = unfinished_parses[: batch_size]
        transitions = model.predict(curr_parses)
        for i in range(len(curr_parses)):
            curr_parses[i].parse_step(transitions[i])
        unfinished_parses = [parse for parse in unfinished_parses if not parse.is_completed()]
